...
これは `c² Δt² / Δx² = 0.5` の 2D 波動方程式です。タップは `falloff²` のくぼみを `current` に足します。ドラッグは始点と終点の間を格子間隔で分割して連続投入します。減衰は粘性スライダーで `0.996`〜`0.886` です。描画は勾配から法線を作り、播散＋ハイライト＋カラーマップで水面に見せます。端は Dirichlet ゼロのため、波は位相が反転して戻ります。
