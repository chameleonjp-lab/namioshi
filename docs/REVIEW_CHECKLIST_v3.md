...
- [済] 時刻付き入力を固定境界のstep終端後へ適用し、20/30/60/120Hzと50/100/150/1000ms停止で得点・内訳・反射系列が一致する
...
- [済] visibility/pagehideはaccumulatorを破棄して境界をrebaseし、非表示時間を物理へ加えず、締切超過復帰は保留stepなしで終了する
...
- [済] ローカルNodeで物理・得点・時刻整合性79件、build、verifyを確認した。GitHub Actions Run #59の67件は基準main時点の記録として保持し、今回変更は別Draftで確認する
