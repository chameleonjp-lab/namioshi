...
- [済] 同じ面を18論理ピクセル移動するまで再判定しない`surfaceHistory`と冷却距離を実装する
- [済] 固定入力診断で、ガラス有限線分の外側では延長線付近で反射しない
- [済] 固定入力診断で、有限線分の端点へ実距離で到達した場合だけ反射する
- [済] 固定入力診断で、同じ時間の反射イベントが60分割37件、120分割36件になる問題をPhase 4Cの固定更新で解消した
- [済] 同じ候補C・同じ3タップ・同じ5秒で、60Hz相当と120Hz相当の得点、得点内訳、反射系列が一致する
- [済] Worldをresetすると波IDの連番が同じ初期値へ戻る
- [済] 子波が親の半径、年齢、寿命を引き継ぐ
- [済] 24波上限で既存波を削除せず、新しい波を拒否する
...
- [済] 累積時間を50msへ制限し、余分な累積を捨てる
- [済] プレイ開始時の単調増加時計から30秒の締切を作る
- [済] Worldの物理更新とゲーム時間の減算を分離した
- [済] 60Hz相当と120Hz相当で公式Worldの論理結果が一致する
