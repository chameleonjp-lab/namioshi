...
`steps^0.32` で分割数に応じて振幅を下げ、速いフリックが帯状に飽和するのを防ぎます。マルチタッチは `pointerId` ごとの最終格子点を `Map` で持ちます。
...
## 11. namioshiへの取り込み状態（PR #82 Draft）

`src/render/ripple-surface.js`へ、判定から独立した2バッファ高さ場を実装した。根タップは`drop`、実際の反射接触と反射波タップは弱い補助dropとして投入し、同じイベントを一度だけ消費する。WebGLは低解像度RGBAテクスチャ、Canvas 2Dは同じピクセル列を透明ハイライトとして描画する。

取り込み時の意図的な差分は、ゲームの論理座標360×640を高さ場の格子へ写像し、品質別セル上限を設けたこと、ゲームのpointer入力を高さ場へ直接渡さず`World.waves`／反射エフェクト／反射波タップ台帳から視覚イベントだけを抽出すること、そして表示を背景へ限定したことである。円環の速度・寿命・半径、有限反射弧、ビーコン判定、得点、終了条件は高さ場から参照しない。

原典の式と実装へ差異が出ないよう、粘性28の値は式`0.996 - 0.28 * 0.11`から`0.9652`とした（以前の表にあった`0.96512`は算術上の誤記）。G2と実機受入は本Draft作成後に別途確認し、マージは行わない。
