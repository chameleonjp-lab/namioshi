...
test('play status explains the objective and the ten-tap wait state',()=>{
  assert.match(main,/function playStatusText\(\)\{[\s\S]*?const pendingDisplayWaves=[\s\S]*?world\.reflectionEffects\?\.length\?\?0[\s\S]*?world\.taps>=MAX_TAPS[\s\S]*?pendingDisplayWaves>0/);
