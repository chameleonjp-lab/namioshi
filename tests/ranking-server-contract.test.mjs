...
test('server contract proposal is disabled, versioned, and isolated',()=>{
  ...
  assert.doesNotMatch(sql,/create or replace function public\.submit_score\(/);
  assert.match(sql,/create or replace function public\.submit_namioshi_score_v1\(/);
  assert.match(sql,/p_play_id text/);
  assert.match(sql,/p_rule_version text/);
  assert.match(sql,/p_season text/);
  assert.match(sql,/grant execute on function public\.submit_namioshi_score_v1\([^;]+to anon/i);
...
test('server contract preserves the self-reported boundary',()=>{
  ...
  assert.match(sql,/revoke execute on function public\.submit_namioshi_score_v1\([^;]+from PUBLIC/i);
});
