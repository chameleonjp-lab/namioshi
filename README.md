...
## 現行main（反射波タップ技能ボーナス統合後）

現行main `5a02f5aa337d848fca1a86f93bc94aaee9ba5c2f`はPull Request #68統合後です。根の波は最大10回で、反射後の有限弧を上手くタップすると技能ボーナスが加わります。hard ceilingは11200点（経路10800点＋反射波タップ最大400点）です。競技ルールは`namioshi-v3-reflection-tap-004`、client versionは`namioshi-v3.4.0-official005`、seasonは`prelaunch-v4`です。旧ルールの端末ベストやランキング記録は新ルールへ移行しません。
...
次のDraftでは、未使用の有限反射弧へ淡い黄色の補助表示を重ね、タップ対象を見つけやすくします。技能ボーナス取得後、根波確定後、寿命終了後は補助表示を消し、得点と物理は変更しません。
