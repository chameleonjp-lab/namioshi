...
現行mainのマージコミットは`b329fb1ebcb1c19af3a12e436de9ec6855a03f52`です。PR #85/#86はマージ済みで、次の修正はDraft PRで確認します。
...
PR #86で、描画が一時的に遅れて反射エフェクトを発生から80ms以上経って初めて観測した場合でも、寿命内ならHAMENへ一度だけ投入するよう補修しました。これにより、遅延だけを理由にリアル水面波が欠落する経路を減らします。物理・得点・入力上限は変更しません。PR #86はmainへマージ済みですが、iPhone 17 Pro Safariでの実表示は未確認です。
