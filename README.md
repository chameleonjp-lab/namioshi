...
現行mainは、根の波10回、反射した水面波のタップ加点、反射回数のHUD／RESULT表示、反射後の表示・タップ対象としての最大10秒保持、プレイ中説明の任意開閉を含みます。波の色分けは廃止し、水面に合わせた一色の波表現へ統一しました。反射した地点のHAMEN局所水面波を第一判定対象とし、固定stepの入力遅延時は波IDとエフェクトIDを保持して同じ対象へ反応します。反射子波の物理・得点処理は従来どおり約3秒で止まり、hard ceilingは11200点（経路10800点＋反射波タップ最大400点）です。競技ルールは`namioshi-v3-reflection-wave-lifetime-005`、client versionは`namioshi-v3.5.0-official006`、seasonは`prelaunch-v5`です。旧ルールの端末ベストやランキング記録は新ルールへ移行しません。
...
反射波タップは、pointerdown時に見えていた輪を入力キューへ記録し、固定stepの遅延で輪が移動しても同じ輪へ反応します。短い表示フェード中の輪も、見えている間はタップ対象です。画面回転やSafariの表示領域変化では、入力と描画のviewportを同期します。WebGLの高さ場テクスチャはCanvas 2Dと同じ上下方向へそろえています。
...
PR #73で通常波の約3秒寿命と10回後の精算を追加し、PR #77で反射後の輪の表示・反射波タップ対象だけを最大10秒へ延長しました。物理・得点の寿命は約3秒のままです。反射輪が残る間は表示中の波の精算を待ちます。得点式とhard ceilingは変更しません。
...
通常の根波は約3秒で自然に消え、反射後の輪は表示・技能ボーナスのタップ対象として最大10秒まで残ります。反射子波の物理・得点処理は約3秒以内です。タップしても出ている波は消えません。根波を10回使い切った後、反射輪を含む表示中の波と入力がなくなれば、30秒を待たずに結果へ進みます。カウントダウン中は説明を表示し、PLAYING中は上部の「説明を表示」ボタンで任意に開閉します。自動検査はActionsで確認中で、実ブラウザとiPhone・iPadでの視認性・終了タイミングは未確認です。
