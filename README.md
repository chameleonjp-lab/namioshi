...
Phase 8Bでは、同じ公開候補コミットでブラウザ表示、スマートフォン操作、描画復旧、音・振動、休止復帰、継続動作を確認します。現在のmainはPR #65統合後の `550b09b8d1e04124001d8cf3812c218854a11441` です。PR #60で波速110と二段階の得点台帳、PR #61で初回案内の成功確認と練習配置の安全制約、PR #62で同時確定時の複数反射種別の集約通知、PR #63で結果画面の最高経路と次の目標、PR #64でプレイ中の波・反射板・ビーコンの見分け方、PR #65で目的・得点条件・6回後の待機状態を統合しました。次のDraftでは、命中確認と得点確定のタイミングをプレイ中へ明示します。G4・G7・G8、iPhone実機、ランキング再開、公開判定は未完了です。記録欄と未確認範囲は[`docs/PHASE8B_DEVICE_TEST_REPORT_v3.md`](docs/PHASE8B_DEVICE_TEST_REPORT_v3.md)で管理します。
