...
## 現行main（反射波タップ視認補助統合後）

現行main `771ee93e0d2ff073b762a2095a048f9da6d1e9e0`はPull Request #69統合後です。根の波は最大10回で、反射後の有限弧を上手くタップすると技能ボーナスが加わります。盤面では未使用の黄色い反射弧を補助表示し、利用可能な間だけ「弧に重ねてタップする」案内を表示します。hard ceilingは11200点（経路10800点＋反射波タップ最大400点）です。競技ルールは`namioshi-v3-reflection-tap-004`、client versionは`namioshi-v3.4.0-official005`、seasonは`prelaunch-v4`です。旧ルールの端末ベストやランキング記録は新ルールへ移行しません。
...
次のDraftでは、反射弧が現れた後のタイミングを凡例内の短い案内で伝えます。対象がなくなったら案内を消し、得点と物理は変更しません。
...
現行mainでは、反射波の輪が見えていても、初見の人がタップ対象を見つけやすいかは未確認です。次のDraftでは未使用の有限反射弧だけへ黄色の補助表示を加え、技能ボーナス取得後や根波確定後には表示しない契約を追加します。実ブラウザ、iPhone・iPad、人の初見再現性、ランキング再開、Supabase適用、Ready化は未確認または未承認のまま維持します。
