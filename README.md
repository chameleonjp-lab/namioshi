...
## 現行main（PR #95マージ後・2026-09-08）

現行mainのマージコミットは`c1830938fe0fd2d4f59397881b04b254f83ee0f8`です。PR #95で公開ページの公式導線を`https://chameleonjp-lab.github.io/chameleonjp_lab/`へそろえました。

PR #95マージ後のG2 Build Verification #214（Run ID `33374104782`）とGitHub Pages build and deployment #64（Run ID `33374103972`）は成功しています。

公開版をCloud Chromeでスモークし、HOME、名前必須ゲート、初回案内の任意タップ、カウントダウン、PLAYINGの10回入力、RESULT遷移、結果内訳、反射回数、端末ベスト保存を確認しました。コンソールのwarning/errorは0件でした。

これは公開版Cloud Chromeの確認であり、iPhone 17 Pro Safariの実機合格ではありません。水面波タップ加点、長いフレーム・表示領域変更後の表示、音・振動、縦画面の視認性、初見操作は未確認のままです。ランキング、Supabase、公式記録、Ready化、公開判定も停止しています。

## 過去のmain記録（PR #91マージ後）



PR #91時点のマージコミットは`c810a2a33259066093d207fa2f585490bf94beef`です。PR #85〜#91の履歴を記録しています。
...
PR #91では、反射水面波エフェクトが寿命`0.42秒`に達した瞬間だけ描画上は透明になるのに、入力やHAMEN同期が有効になる境界差を修正しました。表示・入力・表示同期・削除を`0 <= age < life`へ統一し、反射回数、得点、物理寿命、10回制限、表示波10秒、終了待機は変更していません。
